...
#include <set>
#include <getopt.h>
#include "OptionsParserBoostless.hpp"

/** Constructor which includes the options and requirements for the command line interface */
OptionsParser::OptionsParser(int argc, char *argv[]) : argc_(argc), argv_(argv), integration_time(1.0f), interval_time(0.125f), gpus(0), snapshot_time(0.0f), softening(1e-4f), eta(0.01f), snapshot_number(0), length_output_number(5), resume(false), help(false)
{
...
bool OptionsParser::check_options()
{
  ...
  if (resume)
  {
    ...
    if (rfile.is_open())
    {
      while (getline(rfile, line))
      {
        ...
        // Remove any leading # or leading/trailing whitespace from the parameter name
        key.erase(0, key.find_first_not_of("#"));
        key.erase(0, key.find_first_not_of(" "));
        key.erase(key.find_last_not_of(" ") + 1);
        // Remove any leading or trailing whitespace from the parameter value
    ...
    input_filename= config_param["InputFilename"];

    // read output_filename
    output_filename = config_param["OutputFilename"];
    ...
    std::ostringstream s;
    s << std::setw(length_output_number) << std::setfill('0') << snapshot_number;
    std::string snap_name = output_filename + ".snapshot_" + s.str();
  ...
  // buggy. I set it to 5 for convenience
